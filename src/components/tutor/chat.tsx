...
export function ContextChip({ title, kind, hasCircuit }: { title: string; kind: string; hasCircuit: boolean }) {
  return (
    <div className="flex flex-wrap items-center gap-1.5">
      <span className="inline-flex items-center gap-1.5 rounded-md border border-qx-cyan/25 bg-qx-cyan/10 px-2 py-1 text-[10px] font-semibold uppercase tracking-wider text-qx-cyan">
          ...
          <span
            className={`flex shrink-0 items-center justify-center rounded-lg ${
              ...
              m.role === "user"
                ...
                : "bg-gradient-to-br from-qx-violet to-qx-cyan text-white"
            }`}
          >
          ...
          <div className={`min-w-0 max-w-[86%] ${m.role === "user" ? "" : "space-y-2"}`}>
            <div
              className={`whitespace-pre-line rounded-2xl px-3.5 py-2.5 text-sm leading-relaxed ${
                m.role === "user"
                  ? "rounded-tr-sm bg-qx-violet/15 text-slate-100"
                  : "rounded-tl-sm border border-white/10 bg-ink-900/70 text-slate-200"
              }`}
            >
              {m.mode && m.role === "bot" && (
                <span className="mb-1.5 block text-[10px] font-semibold uppercase tracking-wider text-qx-cyan">
                  {TUTOR_MODES.find((t) => t.id === m.mode)?.label}
                </span>
              )}
              {m.text}
              {m.rephraseLabel && (
                <span className="mt-2 block text-[10px] font-semibold uppercase tracking-wider text-qx-cyan">
                  {m.rephraseLabel}
                </span>
              )}
            </div>
            ...
            {m.grounded && m.grounded.length > 0 && (
              <div className="rounded-xl border border-white/10 bg-ink-950/60 p-3">
                ...
                <ul className="mt-2 space-y-1">
                  {m.grounded.map((g, i) => (
                    <li key={i} className="text-xs leading-relaxed text-slate-400">• {g}</li>
                  ))}
                </ul>
              </div>
            )}
              ...
              <div className="flex flex-wrap gap-1.5">
                {m.followUps.map((f) => (
                  <button
                    ...
                    disabled={typing}
                    className="rounded-full border border-qx-cyan/30 bg-qx-cyan/10 px-2.5 py-1 text-xs font-medium text-qx-cyan transition hover:bg-qx-cyan/20 disabled:opacity-50"
                  >
                    {f}
                  </button>
                ))}
    ...
    <div className={`flex flex-wrap gap-1.5 ${className}`}>
      {prompts.map((p) => (
        <button
          ...
          onClick={() => onPick(p)}
          disabled={disabled}
          className="rounded-full border border-white/10 bg-white/[0.04] px-2.5 py-1 text-xs text-slate-300 transition hover:border-qx-violet/40 hover:bg-qx-violet/10 hover:text-white disabled:opacity-50"
        >
          {p}
        </button>
      ))}
...
export function TutorRephraseRow({
  ...
  return (
    <div className="flex flex-wrap items-center gap-1.5 border-t border-white/5 bg-ink-950/50 px-3 py-2">
      ...
      {options.map(([km, label]) => (
        <button
          ...
          disabled={disabled}
          className="rounded-full border border-white/10 bg-white/5 px-2.5 py-1 text-[11px] font-medium text-slate-300 transition hover:border-qx-violet/40 hover:text-qx-violet disabled:opacity-50"
        >
          {label}
        </button>
      ))}
...
export function TutorComposer({
  ...
  return (
    <form
      ...
      className={`flex items-center gap-2 border-t border-white/5 ${compact ? "p-2.5" : "p-3"}`}
    >
      <input
        value={input}
        ...
        className="min-w-0 flex-1 rounded-xl border border-white/10 bg-ink-900/80 px-3.5 py-2.5 text-sm text-white placeholder:text-slate-600 focus:border-qx-violet/60 focus:outline-none focus:ring-2 focus:ring-qx-violet/20"
      />
      <button
        type="submit"
        disabled={!input.trim() || typing}
        className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-gradient-to-r from-qx-violet to-qx-indigo text-white shadow-glow transition hover:opacity-90 disabled:opacity-40"
        aria-label="Send"
      >
