    ...
    <div className="grid gap-6 lg:grid-cols-2">
      <Card className="p-6">
        <div className="mb-4 flex items-center justify-between">
          <h4 className="font-semibold text-white">The entangled pair</h4>
          <span className="font-mono text-sm text-slate-400">(|00⟩ + |11⟩)/√2</span>
        </div>

        {/* the two qubit cards */}
        <div className="flex items-center justify-center gap-4">
          <QubitCard label="Qubit 0 · Alice" value={justMeasured?.a ?? null} color="violet" />
          <svg width="48" height="24" viewBox="0 0 48 24" className="text-slate-500">
      ...
      <Card className="p-6">
        <h4 className="font-semibold text-white">Statistics over runs</h4>
        <div className="mt-4 grid grid-cols-4 gap-2 text-center">
          {[["00", "violet"], ["01", "rose"], ["10", "rose"], ["11", "cyan"]].map(([st, col]) => {
            ...
            return (
              <div key={st} className="rounded-xl border border-white/10 bg-ink-900/60 p-3">
        ...
        {runs.length === 0 && (
          <p className="mt-4 rounded-lg border border-dashed border-white/10 py-6 text-center text-xs text-slate-500">
...
function QubitCard({ label, value, color }: { label: string; value: 0 | 1 | null; color: "violet" | "cyan" }) {
  ...
  return (
    <div className={`w-32 rounded-2xl border ${value === null ? "border-white/10" : p.ring} bg-ink-900/70 p-4 text-center transition-colors`}>
      <div className="mx-auto mb-3 flex h-16 w-16 items-center justify-center rounded-full border border-white/15 bg-white/5">
        {value === null ? (
