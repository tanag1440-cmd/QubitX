...
export function BlochSphere() {
  ...
  useEffect(() => {
    ...
    const ring = new THREE.Mesh(
      ...
      new THREE.MeshBasicMaterial({ color: 0x33436b, side: THREE.DoubleSide, transparent: true, opacity: 0.8 })
    );
    ...
    const origin = new THREE.Mesh(new THREE.SphereGeometry(0.03, 8, 8), new THREE.MeshBasicMaterial({ color: 0x64748b }));
    scene.add(origin);
    ...
    <div className="grid gap-6 lg:grid-cols-[1.5fr_1fr]">
      <Card className="p-5">
        ...
        <div className="mt-4 flex flex-wrap justify-center gap-2">
          ...
          <button onClick={reset} className="rounded-xl border border-white/10 bg-white/5 px-4 text-sm text-slate-300 hover:bg-white/10">
            Reset
          </button>
        </div>
        <div className="mt-4 flex flex-wrap justify-center gap-2">
          <button onClick={() => setState([{ re: 1, im: 0 }, { re: 0, im: 0 }])} className="rounded-lg border border-white/10 bg-white/5 px-3 py-1.5 font-mono text-xs text-slate-300 hover:bg-white/10"><Ket value={0} /></button>
          <button onClick={() => setState([{ re: 0, im: 0 }, { re: 1, im: 0 }])} className="rounded-lg border border-white/10 bg-white/5 px-3 py-1.5 font-mono text-xs text-slate-300 hover:bg-white/10"><Ket value={1} /></button>
        </div>
      </Card>
      ...
      <div className="space-y-4">
        <Card className="p-5">
          ...
          <div className="mt-4 grid grid-cols-3 gap-2 text-center font-mono text-xs">
            <div className="rounded-lg bg-ink-900/70 p-2"><p className="text-rose-400">x</p><p className="text-white">{b.x.toFixed(2)}</p></div>
            <div className="rounded-lg bg-ink-900/70 p-2"><p className="text-qx-mint">y</p><p className="text-white">{b.y.toFixed(2)}</p></div>
            <div className="rounded-lg bg-ink-900/70 p-2"><p className="text-indigo-300">z</p><p className="text-white">{b.z.toFixed(2)}</p></div>
          </div>
        ...
        <Card className="p-5">
          ...
          {log.length === 0 ? (
            ...
            <ul className="mt-2 space-y-2">
              {log.map((m, i) => (
                <li key={i} className="text-sm text-slate-300">{i === 0 ? "→ " : ""}{m}</li>
              ))}
            </ul>
          )}
        </Card>
