...
export default function Admin() {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6">
      ...
      <div className="mb-6 flex flex-wrap gap-1 rounded-2xl border border-line bg-card p-1.5">
        {tabs.map(([id, label]) => (
          <button
            key={id}
            onClick={() => setTab(id)}
            className={`flex-1 rounded-xl px-4 py-2 text-sm font-semibold transition ${tab === id ? "bg-qx-violet/20 text-qx-violet" : "text-ink-2 hover:bg-card2 hover:text-ink"}`}
          >
            {label}
          </button>
        ))}
      ...
      {tab === "overview" && (
        <>
          ...
          <div className="mt-6 grid gap-6 lg:grid-cols-2">
              ...
              <div className="space-y-2.5">
                {[...LESSONS].sort((a, b) => lessonCompletions(b.id) - lessonCompletions(a.id)).slice(0, 6).map((l) => (
                  <div key={l.id} className="flex items-center gap-3">
                    <span className="w-6 text-right font-mono text-xs text-ink-3">{lessonCompletions(l.id)}</span>
                    <div className="h-5 flex-1 overflow-hidden rounded bg-card2">
                      <div className="h-full rounded bg-accent/80" style={{ width: `${Math.min(100, lessonCompletions(l.id) * 20)}%` }} />
                    </div>
                    <span className="w-44 truncate text-right text-xs text-ink-2">{l.title}</span>
                  </div>
                ))}
              </div>
            </Card>
            <Card className="p-6">
              <h3 className="mb-4 flex items-center gap-2 font-semibold text-ink">
                <GraduationCap className="h-4.5 w-4.5 text-accent" /> Quiz analytics
              </h3>
              {db.quizAttempts.length === 0 ? (
                <p className="text-sm text-ink-3">No quiz attempts recorded yet.</p>
              ) : (
                <div className="space-y-2.5">
                  {LESSONS.map((l) => {
                    ...
                    return (
                      <div key={l.id} className="flex items-center gap-3">
                        <span className="w-40 truncate text-xs text-ink-2">{l.title}</span>
                        ...
                        <span className="w-10 font-mono text-xs text-ink">{avg}%</span>
                      </div>
                    );
                  })}
                </div>
      ...
      {tab === "users" && (
        <Card className="overflow-x-auto">
          <table className="w-full min-w-[640px] text-left text-sm">
            ...
            <tbody>
              {users.map((u) => (
                <tr key={u.id} className="border-b border-line hover:bg-card2/50">
                  <td className="px-5 py-3">
                    <div className="flex items-center gap-2.5">
                      <span className="h-7 w-7 rounded-full" style={{ background: u.avatarColor }} />
                      <div>
                        <p className="font-medium text-ink">{u.name} {u.isAdmin && <Badge color="amber"><Shield className="h-3 w-3" /> admin</Badge>} {u.id === "demo-user" && <Badge color="cyan">demo</Badge>}</p>
                        <p className="text-xs text-ink-3">{u.email}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-5 py-3 text-ink-2">{u.level}</td>
                  <td className="px-5 py-3 font-mono text-ink-2">{db.lessonProgress.filter((p) => p.userId === u.id && p.status === "completed").length}/10</td>
                  <td className="px-5 py-3 font-mono text-qx-amber">{totalXpByUser(u.id)}</td>
                  <td className="px-5 py-3 font-mono text-ink-2">{db.quizAttempts.filter((a) => a.userId === u.id).length}</td>
                  <td className="px-5 py-3 text-right">
                    {isAdmin && !u.isAdmin && u.id !== currentUser.id ? (
                      <button onClick={() => { if (confirm(`Delete ${u.name}? This removes all their progress.`)) deleteUser(u.id); }} className="rounded-lg p-1.5 text-ink-3 hover:bg-rose-500/10 hover:text-rose-400" aria-label={`Delete ${u.name}`}>
      ...
      {tab === "content" && (
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <h3 className="flex items-center gap-2 font-semibold text-ink"><BookOpen className="h-4.5 w-4.5 text-qx-violet" /> Lessons & quiz questions</h3>
              ...
              return (
                <Card key={l.id} className="p-5">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <p className="text-xs text-ink-3">Module {moduleById(l.moduleId)?.order} · {l.duration}</p>
                      <h4 className="mt-0.5 font-semibold text-ink">{l.title}</h4>
                    </div>
                    <Badge color={l.id === "l3" ? "cyan" : "slate"}>+{l.xp} XP</Badge>
                  </div>
                  <div className="mt-3 flex flex-wrap gap-2 text-xs">
                    <span className="rounded-lg bg-card2 px-2.5 py-1 text-ink-2">
                      {LESSONS.filter((x) => x.id === l.id)[0].sections.length} sections
                    </span>
                    <span className="rounded-lg bg-card2 px-2.5 py-1 text-ink-2">{l.quiz.length} quiz questions</span>
                    <span className="rounded-lg bg-card2 px-2.5 py-1 text-ink-2">{lessonCompletions(l.id)} completions</span>
                    {avg !== null && (
                  ...
                  <details className="mt-3">
                    <summary className="cursor-pointer text-xs font-semibold text-accent hover:underline">Preview quiz questions</summary>
                    <ul className="mt-2 space-y-2">
                      {l.quiz.map((q) => (
                        <li key={q.id} className="rounded-lg border border-line bg-card2/60 p-2.5 text-xs text-ink-2">
                          <span className="text-ink-2">{q.question}</span>
                          <span className="mt-1 block font-mono text-qx-mint">✓ answer: {q.options[q.correctIndex]}</span>
                        </li>
                      ))}
                    </ul>
                  </details>
                  <Link to={`/learn/${l.id}`} className="mt-3 inline-block text-xs font-semibold text-ink-2 hover:text-ink">Preview lesson →</Link>
                </Card>
              );
            })}
          </div>
          <div className="flex flex-wrap items-center gap-3 rounded-2xl border border-dashed border-line p-5">
            <BookPlus className="h-5 w-5 text-ink-3" />
            <p className="text-sm text-ink-2">
              <span className="font-semibold text-ink">Lesson authoring</span> — the content schema supports adding
      ...
      {tab === "resources" && (
        <div className="grid gap-6 lg:grid-cols-[360px_1fr]">
          <Card className="h-fit p-5">
            <h3 className="mb-3 flex items-center gap-2 font-semibold text-ink"><Plus className="h-4 w-4 text-accent" /> Add a resource</h3>
            <form onSubmit={submitResource} className="space-y-3">
              <input
                ...
                placeholder="Title"
                required
                className="w-full rounded-xl border border-line bg-card2 px-3 py-2 text-sm text-ink placeholder:text-ink-3 focus:border-qx-violet/60 focus:outline-none"
              />
              <input
              ...
              <div className="grid grid-cols-2 gap-2">
                <select
                  ...
                  className="rounded-xl border border-line bg-card2 px-2 py-2 text-sm text-ink focus:outline-none"
                >
              ...
              {!isAdmin && <p className="text-center text-xs text-ink-3">Requires admin rights</p>}
            </form>
          </Card>

          <div>
            <p className="mb-3 text-sm text-ink-3">{RESOURCES.length + (db.resources?.length ?? 0)} resources · {db.resources?.length ? `${db.resources.length} curated by you` : "seed library"}</p>
            <div className="grid gap-3 sm:grid-cols-2">
              {(db.resources ?? []).map((r) => (
                <div key={r.id} className="flex items-center gap-3 rounded-xl border border-qx-cyan/25 bg-qx-cyan/5 p-3.5">
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-ink-3">{r.type} · {r.category}</p>
                  </div>
                  {isAdmin && (
                    <button onClick={() => deleteResource(r.id)} className="rounded-lg p-1.5 text-ink-3 hover:bg-rose-500/10 hover:text-rose-400" aria-label="Delete resource">
                      <Trash2 className="h-4 w-4" />
                    </button>
                  )}
                </div>
              ...
              {RESOURCES.slice(0, 6).map((r) => (
                <div key={r.id} className="rounded-xl border border-line-strong/8 bg-card2/50 p-3.5 opacity-70">
                  <p className="truncate text-sm font-medium text-ink-2">{r.title}</p>
                  <p className="text-xs text-ink-3">{r.type} · {r.category} · seed</p>
                </div>
              ))}
            </div>
          </div>
