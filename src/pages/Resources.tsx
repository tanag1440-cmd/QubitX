...
const TYPE_COLORS: Record<string, string> = {
  Article: "text-accent bg-qx-cyan/10 border-qx-cyan/30",
        ...
        {CATEGORIES.map((c) => (
          <button
            ...
            className={`rounded-full border px-3.5 py-1.5 text-sm font-medium transition ${
              category === c ? "border-qx-violet/60 bg-qx-violet/20 text-qx-violet" : "border-line bg-card2/50 text-ink-2 hover:bg-card2"
            }`}
          >
            {c}
          </button>
        ))}
        <span className="mx-2 h-5 w-px bg-card2" />
        {TYPES.map((t) => (
          <button
            key={t}
            onClick={() => setType(t)}
            className={`rounded-full border px-3 py-1 text-xs font-semibold transition ${
              type === t ? "border-qx-cyan/60 bg-qx-cyan/15 text-accent" : "border-line bg-card2/50 text-ink-3 hover:bg-card2"
            }`}
          >
            {t}
          </button>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((r) => (
            <a
              ...
              className="group flex flex-col rounded-2xl border border-line bg-card/80 p-5 transition hover:border-qx-violet/40 "
            >
              <div className="flex items-start justify-between gap-3">
                <span className={`flex h-9 w-9 items-center justify-center rounded-xl border ${TYPE_COLORS[r.type]}`}>
                  {TYPE_ICON[r.type]}
                </span>
                <ArrowUpRight className="h-4 w-4 text-ink-3 transition group-hover:text-accent" />
              </div>
              <h3 className="mt-3 font-semibold leading-snug text-ink group-hover:text-accent">{r.title}</h3>
              <p className="mt-1.5 flex-1 text-sm leading-relaxed text-ink-2">{r.description}</p>
